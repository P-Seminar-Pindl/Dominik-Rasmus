...
func _select_building(res: BuildingResource) -> void:
	...
	if res == null:
		_status_label.text = ""
		return
	_status_label.text = "Placing: " + res.name + "   [RMB] cancel"
...
func _process(_delta: float) -> void:
	...
	_preview_valid  = _can_place(_current_tile, _selected_res.footprint_size)

	var fp := _selected_res.footprint_size
...
func _place_building(anchor: Vector2i, res: BuildingResource) -> void:
	...
	placed_buildings[anchor] = {"resource": res, "node": wrapper}
	for dx in range(fp.x):
		for dz in range(fp.y):
